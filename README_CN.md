...
- **裁决规则集 v1(2026-07)** —— LLM 提出的 critical 现在必须记录 `alternative_explanation_checked`(排除了哪些良性解读:舍入、单位、统计口径;缺失 → 降为 major);每条存活的 critical 还会经过一次 fresh-thread 反驳 pass,其全部权力只是给人看的 ⚠️ CONTESTED 标记(永不改动严重度或判决)。报告标记 `adjudicator: deterministic-rules-v1`;此字段出现之前生成的 findings 文件若直接重新裁决,这类 critical 会被降级(HARD_FLAGS → SOFT_FLAGS)——请重跑 auditor,不要重裁旧 findings。
