...
- **裁决规则集 v1(2026-07)** —— LLM 提出的 critical 现在必须记录 `alternative_explanation_checked`(排除了哪些良性解读:舍入、单位、统计口径;缺失 → 降为 major);每条存活的 critical 还会经过一次 fresh-thread 反驳 pass,其全部权力只是给人看的 ⚠️ CONTESTED 标记(反驳者永不改动严重度)。**v2 增加唯一合法的降级通道**:adjudicator 对 allow-listed critical 基于冻结的 `numeric_basis` 自动运行确定性区间算术反证(`tools/countercheck.py`)。只有**绑定不变**的计算才有降级权:`HP-NUM-INFLATE` / `HP-APPENDIX-CONTRA`(对 `%` 测量值做对称的显示区间相交检验——角色分配可证明不影响结论,无单位精确量绝不按舍入处理)证明成立时降为 info;`HP-DELTA-ERROR` 公式不对称,**永远只报告不降级**。保守残余:pattern_id 由 auditor 选择,错标只可能保住 flag、绝不可能消除 flag。报告标记 `adjudicator: deterministic-rules-v2`;此字段出现之前生成的 findings 文件若直接重新裁决,这类 critical 会被降级(HARD_FLAGS → SOFT_FLAGS)——请重跑 auditor,不要重裁旧 findings。
