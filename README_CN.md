...
- **裁决规则集 v1(2026-07)** —— LLM 提出的 critical 现在必须记录 `alternative_explanation_checked`(排除了哪些良性解读:舍入、单位、统计口径;缺失 → 降为 major);每条存活的 critical 还会经过一次 fresh-thread 反驳 pass,其全部权力只是给人看的 ⚠️ CONTESTED 标记(反驳者永不改动严重度)。**v2 增加确定性反证计算——经对抗审后收敛为全员 REPORT-ONLY**:adjudicator 对 allow-listed critical 基于冻结的 `numeric_basis` 自动运行精确区间算术(`tools/countercheck.py`),并把计算过程记录在 finding 和报告里。一个 resolver 要有降级权必须同时满足 (a) 结论对一切模型可控自由度不变、(b) 前提本身可计算——`HP-DELTA-ERROR` 不满足 (a)(old/new 绑定不对称),对称的显示区间检验不满足 (b):"这些数是舍入显示"不可计算("dropout 恰为 50%" vs "50.4%" 是会被舍入逻辑错误开脱的真矛盾)。所以**当前没有任何降级发生**;计算能证明矛盾**存在**(GRIM 家族),但证明矛盾**不可能存在**需要 parser 不具备的语义。真正生效的门:合格 critical 必须冻结 `numeric_basis`,否则降为 major(不可审计的数值指控不配 HARD flag)。报告标记 `adjudicator: deterministic-rules-v2`;此字段出现之前生成的 findings 文件若直接重新裁决,这类 critical 会被降级(HARD_FLAGS → SOFT_FLAGS)——请重跑 auditor,不要重裁旧 findings。
...
**作者侧自查(已评估,范围受限)。**
- **academic-integrity-skill** —— 1anj。一个作者侧、湿实验/生物医学的自查 skill(重图像取证)。我们评估了它的非图像确定性筛查 —— 原始数据尾数 / 精确重复取证(last-digit / Benford 传统)与 reported-vs-source 对账 —— 两者都未采纳为检查项:尾数取证需要审稿侧分层很少能拿到的原始 per-sample 数据表,reported-vs-source 又与家族 D 重叠。在此致谢为先行工作,也是本审稿侧工具箱的作者侧对偶。*(MIT。)*
- **anti-defensive-writing** —— Kiterlin。一个作者侧 Codex skill,*改写*防御性写作(删去多余 caveat/对冲、强化文风)。它是我们 `AIS-DEFENSIVE-HEDGE` 的干净**对偶**:他们替作者改、我们替审稿人标(零权重)。我们参照它的 discouraged-construction 清单扩了我们的确定性 hedge 模板("this is not to say"、"this should not be taken to mean"、"rather than arguing X, we argue Y")。*(MIT。)*
...
| [1anj/academic-integrity-skill](https://github.com/1anj/academic-integrity-skill) | 51 | 2026-05 | **定位上最近的同类 skill,但立场相反**:一个**作者侧**湿实验/生物医学自查 skill(重图像取证 —— blot/显微镜/流式,正是我们排除的)。作者投稿前自查 vs 我们审计第三方 autoresearch 产出;其数值/引用筛查跑在作者手里的原始数据表上,而非审稿侧的 PDF/LaTeX。*(MIT)* |
| [Kiterlin/anti-defensive-writing](https://github.com/Kiterlin/anti-defensive-writing) | 10 | 2026-06 | 我们 `AIS-DEFENSIVE-HEDGE` 的作者侧**对偶**:一个*改写*防御性写作的 Codex skill(caveat/对冲 → 直接、claim-forward 的表达)。他们投稿前改、我们审稿侧零权重标记;我们参照其 discouraged-construction 清单扩了 hedge 模板。*(MIT)* |
