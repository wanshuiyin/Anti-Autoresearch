...
- **裁决规则集 v1(2026-07)** —— LLM 提出的 critical 现在必须记录 `alternative_explanation_checked`(排除了哪些良性解读:舍入、单位、统计口径;缺失 → 降为 major);每条存活的 critical 还会经过一次 fresh-thread 反驳 pass,其全部权力只是给人看的 ⚠️ CONTESTED 标记(反驳者永不改动严重度)。**v2 增加唯一合法的降级通道**:adjudicator 对 `HP-DELTA-ERROR` / `HP-NUM-INFLATE` / `HP-APPENDIX-CONTRA` 的 critical 自动运行确定性区间算术反证(`tools/countercheck.py`),基于其冻结的 `numeric_basis`——由**计算**证明矛盾不可能成立时降为 info,全程无模型输出参与。报告标记 `adjudicator: deterministic-rules-v2`;此字段出现之前生成的 findings 文件若直接重新裁决,这类 critical 会被降级(HARD_FLAGS → SOFT_FLAGS)——请重跑 auditor,不要重裁旧 findings。
