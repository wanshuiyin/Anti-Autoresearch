...
- **Adjudicator ruleset v1 (2026-07)** — LLM-proposed criticals now must record `alternative_explanation_checked` (what benign readings — rounding, units, statistical conventions — the reviewer ruled out; missing → demoted to major), and each surviving critical gets a fresh-thread refutation pass whose only power is a ⚠️ CONTESTED marker for the human (the refuter never moves severity). **v2 adds the one sanctioned demotion path**: the adjudicator auto-runs deterministic interval-arithmetic counter-checks (`tools/countercheck.py`) on allow-listed criticals over their frozen `numeric_basis`. Only a **binding-invariant** computation may demote: `HP-NUM-INFLATE` / `HP-APPENDIX-CONTRA` (a symmetric display-interval check on `%`-measurements — role assignment provably cannot change the verdict, and exact unitless quantities are never treated as rounded) demote to info on proof; `HP-DELTA-ERROR`'s asymmetric formula is REPORT-ONLY forever (its result informs the human, never the severity). Residual, deliberately conservative: the auditor picks the `pattern_id`, so mislabeling can only KEEP a flag, never remove one. Reports stamp `adjudicator: deterministic-rules-v2`; findings files produced before this field existed will see such criticals demoted (HARD_FLAGS → SOFT_FLAGS) when re-adjudicated — re-run the auditors, don't re-adjudicate stale findings.
