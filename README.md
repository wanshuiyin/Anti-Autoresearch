...
- **Adjudicator ruleset v1 (2026-07)** — LLM-proposed criticals now must record `alternative_explanation_checked` (what benign readings — rounding, units, statistical conventions — the reviewer ruled out; missing → demoted to major), and each surviving critical gets a fresh-thread refutation pass whose only power is a ⚠️ CONTESTED marker for the human (the refuter never moves severity). **v2 adds deterministic counter-checks — which turned out, under adversarial review, to be REPORT-ONLY across the board**: the adjudicator auto-runs exact interval-arithmetic resolvers (`tools/countercheck.py`) on allow-listed criticals over their frozen `numeric_basis`, and records the computation on the finding and in the report. A resolver could demote only if its verdict were (a) invariant under every model-controlled degree of freedom AND (b) built on computable premises — `HP-DELTA-ERROR` fails (a) (asymmetric old/new binding), and the symmetric display-interval check fails (b): "these numbers are rounded displays" is not computable (`dropout of exactly 50%` vs `50.4%` is a real contradiction rounding logic would wrongly excuse). So **nothing demotes today**; a computation may prove a discrepancy EXISTS (the GRIM family), but proving one CANNOT exist needs semantics no parser has. The gate that does act: an eligible critical must freeze its `numeric_basis` or demote to major (unauditable numeric accusations may not be HARD flags). Reports stamp `adjudicator: deterministic-rules-v2`; findings files produced before this field existed will see such criticals demoted (HARD_FLAGS → SOFT_FLAGS) when re-adjudicated — re-run the auditors, don't re-adjudicate stale findings.
